...
class AwardedJob(Base):
    ...
    print_material_spec: Mapped[str | None] = mapped_column(String(160), index=True)
    print_thickness: Mapped[float | None] = mapped_column(Float)
    print_gdt_callout_count: Mapped[int | None] = mapped_column(Integer)
    print_tolerance_count: Mapped[int | None] = mapped_column(Integer)
